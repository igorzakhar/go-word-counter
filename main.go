...
func main() {
	...
	go RunConcurrentTasks(word, done, resultChannel, maxGoroutines)
...
func RunConcurrentTasks(word string, done chan bool, resultChan chan *result, concurrencyLimit int) {
...
func GetWordCount(url, word string) result {
	...
	wordCount := bytes.Count(body, []byte(word))
